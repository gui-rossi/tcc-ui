    ...
    <div style="margin-left: 4px;">
      <div class="row">
        ...
        <div style="margin-left: 1em;">
          <!-- <v-time-picker v-model="picker" :landscape="true">Teste</v-time-picker> -->
          <span>Start Time: </span>
          <v-btn  class="time-button" @click="selectStartTime">{{ startTime }}</v-btn>
          <span class="span-2">End Time: </span>
          <v-btn class="time-button" @click="selectEndTime">{{ endTime }}</v-btn>
        </div>
      </div>
      <div class="row">
        <div class="light-time">Person detection time window:</div>
        <div style="margin-left: 1em;">
          <span>Start Time: </span>
          <v-btn class="time-button" @click="selectStartDetectionTime">{{ startTimeDetection }}</v-btn>
          <span class="span-2">End Time: </span>
          <v-btn class="time-button" @click="selectEndDetectionTime">{{ endTimeDetection }}</v-btn>
        </div>
      </div>
      <div class="row">
      ...
      <div class="row">
        ...
        <v-switch class="status-switch" color="secondary" :model-value="true" v-model="this.isLightOn" label="Light" hide-details></v-switch>
      </div>
      <div class="row">
        <!-- <input type="checkbox" id="alarmPlaying" v-model="isAlarmPlaying">
        <label for="alarmPlaying">Alarm Status</label> -->
        <v-switch class="status-switch" color="secondary" :model-value="true" v-model="isAlarmPlaying" label="Sound Alarm Now" hide-details></v-switch>
      </div>
    </div>
    <div style="margin-top: auto;">
      <v-btn  density="comfortable" @click="saveConfig">Save</v-btn>
    </div>
  </div>
</template>
